...
final class ChainTests: XCTestCase {
  ...
  func testChainIndexTraversals() {
    ...
    validator.test(chain("abcd", "XYZ"), expectedCount: 4 + 3)
    validator.test(chain("abcd", ""), expectedCount: 4 + 0)
    validator.test(chain("", "XYZ"), expectedCount: 0 + 3)
    validator.test(chain("", ""), expectedCount: 0 + 0)
  }
