...
//    a.index(after: a.endIndex) // Precondition failed: Advancing past end index
  ...
  func testIndexTraversals() {
    do {
      ...
      validator.test(empty.striding(by: 1))
      validator.test(empty.striding(by: 2))
    }
    
    do {
      let range = 0...100
      let validator = IndexValidator<StridingCollection<ClosedRange<Int>>>()
      validator.test(range.striding(by: 10))
      validator.test(range.striding(by: 11))
      validator.test(range.striding(by: 101))
    }
    
    do {
      let array = Array(0...100)
      let validator = IndexValidator<StridingCollection<[Int]>>()
      validator.test(array.striding(by: 10))
      validator.test(array.striding(by: 11))
      validator.test(array.striding(by: 101))
    }
    
    do {
      let string = "swift rocks"
      let validator = IndexValidator<StridingCollection<String>>()
      validator.test(string.striding(by: 1))
      validator.test(string.striding(by: 2))
      validator.test(string.striding(by: 10))
    }
  }
