...
final class ProductTests: XCTestCase {
  ...
  func testProductIndexTraversals() {
    ...
    validator.test(product([1, 2, 3, 4], "abc"), expectedCount: 4 * 3)
    validator.test(product([1, 2, 3, 4], ""), expectedCount: 4 * 0)
    validator.test(product([], "abc"), expectedCount: 0 * 3)
    validator.test(product([], ""), expectedCount: 0 * 0)
  }
}
